...
function ProductCard({ product }: { product: Product }) {
  return (
    <Container className="flex-col overflow-hidden rounded-2xl border border-[#E7E7EA]">
      {/* Image: full-bleed to the card's top/left/right edges, no padding here */}
      <Container className="relative aspect-square w-full bg-[#F3F1EC]">
        {product.recommended && (
          <span className="absolute left-3 top-3 z-10 rounded-full bg-[#9B1B32] px-3 py-1 text-[11px] font-semibold text-white">
            Recommended
          </span>
        )}
        <Media
      ...
      <Container className="flex-col p-4">
        <Container className="items-center justify-between">
          <Typography className="m-0 text-[11px] text-gray-400">
            {product.sku}
          </Typography>
          <StarRating rating={product.rating} count={product.reviewCount} />
        </Container>

        <Typography className="m-0 mt-1.5 font-outfit text-[16px] font-bold leading-snug text-[#24232D]">
          {product.name}
        </Typography>

        <Typography className="m-0 mt-1 text-[13px] leading-snug text-gray-500">
          {product.description}
        </Typography>

        <Container className="mt-2.5 flex-wrap items-center gap-1.5">
          {product.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-[#24232D]/15 px-2.5 py-0.5 text-[11px] font-medium text-[#24232D]"
            >
              {tag}
            </span>
          ))}
        ...
        <Container className="mt-3 border-t border-[#EDEDED] pt-3" />

        <Container className="items-center justify-between">
          <Container className="items-baseline gap-2">
            <Typography className="m-0 font-outfit text-[18px] font-bold text-[#24232D]">
              ${product.price.toLocaleString()}
            </Typography>
            {product.originalPrice && (
              <Typography className="m-0 text-[13px] text-gray-400 line-through">
          ...
          <button
            type="button"
            className="flex cursor-pointer items-center gap-1 rounded-full bg-[#9B1B32] px-4 py-2 text-[13px] font-semibold text-white transition-colors hover:bg-[#7f1628]"
          >
            View
...
// ---------------------------------------------------------------------------
// Filter sidebar (shared between desktop panel and mobile drawer)
// ---------------------------------------------------------------------------
    ...
    <Container className="flex-col gap-5">
      ...
      <Container className="relative">
        ...
        <input
          ...
          className="w-full rounded-2xl border border-[#E7E7EA] bg-white py-2.5 lg:py-3 pl-10 pr-4 text-[14px] text-[#24232D] placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#9B1B32]/30"
        />
      </Container>
      ...
      <Container className="flex-col">
        {FILTER_GROUPS.map((group) => {
          ...
          return (
            <Container
              key={group.id}
              className="flex-col border-b border-[#9B1B32] py-4 last:border-b-0"
            >
              <button
                type="button"
              ...
              {hasOptions && (
                ...
                >
                  <Container className="overflow-hidden">
                    <Container className="flex-col gap-2.5 pt-3">
                      {group.options.map((option) => {
                        ...
                        return (
                          ...
                          >
                            <Container className="items-center gap-2.5">
                              <input
                                type="checkbox"
                                checked={isChecked}
                                onChange={() =>
                                ...
                                className="h-4 w-4 shrink-0 cursor-pointer accent-[#9B1B32]"
                              />
                              <Typography className="m-0  font-inter lg:text-[14px] text-[13.5px] text-[#24232D]">
                                {option.label}
                              </Typography>
                            </Container>
...
export default function ProductListingPage() {
  ...
  return (
    <Container
      component="main"
      className="flex-col bg-white px-4 py-6 sm:px-8 sm:py-8 lg:px-16 lg:py-11"
    >
      <Container width="fullWidth" className="mx-auto max-w-350 flex-col">
        <Container className="items-center font-inter gap-1.5 text-[14px] text-gray-400">
          <span>Home</span>
          <ChevronRight size={13} />
          <span>Printers</span>
          <ChevronRight size={13} />
          <span className="text-[#24232D]">Aluminum Cans</span>
        </Container>

        <Typography className="m-0 mt-2 lg:mt-5 font-outfit font-bold text-[26px] leading-tight text-[#24232D] lg:text-[82px]">
        ...
        {!bannerDismissed && (
          <Container className="relative mt-5 flex-col gap-4 rounded-[20px] bg-[#FDF3F0] px-5 py-4 lg:py-6.5 lg:flex-row lg:items-center lg:justify-between lg:gap-2">
            <Container className="flex-1 flex-col gap-0.5 pr-8 lg:pr-0">
              <Typography className="m-0 text-[14px] font-outfit text-[#9B1B32] font-bold">
                Not sure where to start?
              </Typography>
              <Typography className="mt-2 font-inter text-[14px] lg:text-[13px] text-black">
            ...
            <Container className="items-center justify-start gap-5">
              <button
                type="button"
                className="whitespace-nowrap font-outfit rounded-full bg-[#9B1B32] px-5 py-2.5 text-[14px] font-semibold text-white transition-colors hover:bg-[#7f1628]"
              >
                Start the Quiz
              </button>
        ...
        <Container className="mt-6 lg:mt-5 items-center justify-between">
          <Typography className="font-outfit text-[19px] lg:text-[36px] font-bold text-[#24232D]">
          ...
          <Container className="items-center gap-3">
            ...
            <Container className="relative hidden lg:block">
              <select
                ...
                className="cursor-pointer appearance-none rounded-2xl border border-[#C7C7C7] bg-white py-2 lg:py-2.5 pl-4 pr-9 text-[13px] font-inter lg:text-[16px] text-black focus:outline-none"
              >
        ...
        <Container
          width="fullWidth"
          className="mt-6 flex-col gap-8 lg:gap-10 lg:flex-row lg:items-start"
        >
          {/* Desktop sidebar */}
